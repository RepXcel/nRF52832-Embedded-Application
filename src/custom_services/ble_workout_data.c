...
static void on_write(ble_workout_data_t* p_workout_data, ble_evt_t const* p_ble_evt) {
    ...
    if ((p_evt_write->handle == p_workout_data->custom_value_handles.cccd_handle) && (p_evt_write->len == 2)) {
        ...
        if (p_workout_data->evt_handler != NULL) {
            ...
            evt.evt_type = ble_srv_is_notification_enabled(p_evt_write->data)
                               ? BLE_WORKOUT_DATA_EVT_NOTIFICATION_ENABLED
                               : BLE_WORKOUT_DATA_EVT_NOTIFICATION_DISABLED;
...
static ret_code_t custom_value_char_add(ble_workout_data_t* p_workout_data,
                                        const ble_workout_data_init_t* p_workout_data_init) {
    ret_code_t err_code;
    ...
    err_code = sd_ble_gatts_characteristic_add(p_workout_data->service_handle, &char_md, &attr_char_value,
...
ret_code_t ble_workout_data_init(ble_workout_data_t* p_workout_data,
                                 const ble_workout_data_init_t* p_workout_data_init) {
...
ret_code_t ble_workout_data_value_update(ble_workout_data_t* p_workout_data, workout_data_t custom_value) {
    ...
    // Update database.
    err_code = sd_ble_gatts_value_set(p_workout_data->conn_handle, p_workout_data->custom_value_handles.value_handle,
                                      &gatts_value);
    VERIFY_SUCCESS(err_code);
